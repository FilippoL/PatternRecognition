...
```{r}
principal_components = prcomp(scaled_pixels_data[,-indices_of_useless_features])
```
...
```{r}
density_model3 = multinom(label ~ scaled_density + log_dens, data2, maxit=1000)
density_model3

predicted = predict(density_model3, data2)
confmat = table(data2$label, predicted)
confmat
sum(diag(confmat))/sum(confmat)
```
