...
melted_cormat <- melt(ifelse(!is.finite(log(as.matrix(cormat))+4), 0, log(as.matrix(cormat))+4))
...
```
```{r}
density_model = multinom(label ~ scaled_density, data2, maxit=1000)
...
```{r}
density_model2 = multinom(label ~ scaled_density + squared_dens, data2, maxit=1000)
...
```
```{r}
density_model = multinom(label ~ scaled_density_edge_detected, data, maxit=1000)
predicted = predict(density_model, data)
confmat = table(data$label, predicted)
confmat
sum(diag(confmat))/sum(confmat)
...
boxplot(pca~label, sample_n(data_for_boxplots, 5000), main="PCA", xlab="Classes", ylab="PC value")
```
...
```{r}
data3 = cbind.data.frame(scaled_density = scale(image_density),
                         first_pca = scale(principal_components[,2]),
                        # scaled_density_edge_detected = scale(image_density_edge_detected), 
                        label=mnist.dat$label)
