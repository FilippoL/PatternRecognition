...
#SAMPLING 500 FROM TRAINING, TO BE REMOVED!!!
```{r}
memory.limit(30000)
training_data = sample_n(training_data, 500)
```
...
##Tuned
```{r}
tuned_results_svm = tune.svm(label ~., data = training_data, cost=1:5, gamma = c(0.1, 0.05, 0.0025, 0.00125, 0.000625))
tuned_results_svm$performances

tuned_model_svm = svm(label ~., data = training_data)
...
tuned_results_nnet = tune(nnet, train.x = training_data[,-1], train.y = as.numeric(training_data$label), data = training_data, MaxNWts=5000, ranges =list(size=seq(1,5), decay=c(0.1, 0.05, 0.0025, 0.00125, 0.000625)))
...
#Plotting Accuracies
```{r}
accuracies <- c(11.15, 22.68, 22.9, 29.28, 36.23)
par(mar=c(7, 5, 1, 1))
...
las=2)
namesss = c("Majority classification", "Density only", "Logarithmic nonlinearity", "Edge detection", "Second principle component")
text(xx, y = accuracies, label = accuracies, pos = 3, cex = 0.8, col = "red")
axis(1, at=xx, labels=namesss, tick=FALSE, las=2, line=-0.5, cex.axis=0.5)
